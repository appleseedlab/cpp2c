...
EVALUATION_PROGRAMS = [
    ...
    # after these fixes i could not get make check to work, but i was able
    # to build and run emacs successfully.
    EvaluationProgram(
        r'emacs-28.1',
        r'https://ftp.snt.utwente.nl/pub/software/gnu/emacs/emacs-28.1.tar.gz',
        r'src',
        r'bash configure --with-gnutls=ifavailable && intercept-build make -j64',
        r'''
        make clean                  &&
        make                        &&
        make check
    ...
    # # contains c++ code.
    # # did not transform c++ code.
    # # built on church with 64 cores.
    # # transforms
    # # takes 1 min
    # # failed tests.
    # # problem:  in transformed definition of UNALIGNED_P in sha1.c, the macro
    # #           alignof was used before it was defined.
    # # fix:      moved alignof definition above transformed definition.
    # # problem:  the transformed definition of HTAB_DELETED_ENTRY was #include'd
    # #           in libcpp/files.cpp.
    # #           it seems that c++ doesn't handle redeclarations the same way c
    # #           does, so while this would be legal in c, it is not in cpp.
    # # fix:      change the linkage from static to extern.
    # # problem:  ditt for macro HTAB_EMPTY_ENTRY
    # # problem:  in regex.c, undefined reference to transformed definition of
    # #           re_syntax_options.
    # #           the macro's transformed declaration was emitted, but
    # #           not its transformed definition for some reason.
    # #           this might have to do with the fact re_syntax_options is
    # #           defined as the nested macro xre_syntax_options, which
    # #           is not defined in gcc.
    # # fix:      replace transformed function calls with xre_syntax_options
    # # problem:  error message:
    # #           gengtype: failed to compute output name for
    ...
    #     r'gcc',
    #     r'./configure --disable-multilib && intercept-build make -j64',
    #     r'''
    #     make clean                  &&
    #     make                        &&
    #     make check
