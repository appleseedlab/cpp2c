...
for FILEPATH in $(ls $SRC_DIR/*.c); do
    ...
    $CPP2_C -fsyntax-only -I$SOURCE_DIR -I$SOURCE_DIR/h $FILEPATH -Xclang -plugin-arg-cpp2c -Xclang -overwrite-files -Xclang -plugin-arg-cpp2c -Xclang -dump-stats -Xclang -plugin-arg-cpp2c -Xclang $CSV_DIR/$FN_NO_EXT.csv
done
